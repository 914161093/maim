// D:/lj/haoxingj/haoxingj/node_modules/mobile-select/dist/mobile-select.es.js
import "D:/lj/haoxingj/haoxingj/node_modules/mobile-select/dist/style/mobile-select.css";
var S = Object.defineProperty;
var D = (g, t, e) => t in g ? S(g, t, { enumerable: true, configurable: true, writable: true, value: e }) : g[t] = e;
...
var l = class {
  constructor(t) {
    ...
    h(this, "config");
    !l.checkRequiredConfig(t) || (this.config = Object.assign({}, l.defaultConfig, t), this.wheelsData = t.wheels, this.isJsonType = false, this.cascadeJsonData = [], this.displayJson = [], this.curValue = [], this.curIndexArr = [], this.isCascade = false, this.startY, this.moveEndY, this.moveY, this.preMoveY, this.offsetY = 0, this.offsetSum = 0, this.oversizeBorder, this.curDistance = [], this.enableClickStatus = false, this.optionHeight = 0, this.initPosition = t.position || [], this.initColWidth = t.colWidth || [], this.init());
  }
  init() {
  ...
  static checkRequiredConfig(t) {
    ...
    if (!t) {
      const i = e.map((s) => `'${s}'`);
      return l.log("error", `missing required param ${i.join(" and ")}.`), false;
    }
    for (let i = 0; i < e.length; i++) {
      const s = e[i];
      if (!t[s])
        return l.log("error", `missing required param '${s}'.`), false;
    }
    return true;
  }
  static log(t, e) {
  ...
  checkTriggerAvailable() {
    const { config: t } = this;
    return this.trigger = t.trigger instanceof HTMLElement ? t.trigger : document.querySelector(t.trigger), this.trigger ? true : (l.log("error", "trigger HTMLElement does not found on your document."), false);
  }
  ...
  registerEvents(t) {
    for (const [e, i] of Object.entries(this.eventHandleMap))
      typeof i.event == "string" ? this[e][`${t}EventListener`](i.event, i.fn, { passive: false }) : i.event.forEach((s) => {
        this[e][`${t}EventListener`](s, i.fn, { passive: false });
      });
  }
  destroy() {
    var t, e;
    ...
    if (this.isJsonType) {
      ...
      for (let s = 0; s < i.length; s++)
        if (t.childs in i[s] && ((e = i[s][t.childs]) == null ? void 0 : e.length) > 0)
          return this.cascadeJsonData = this.wheelsData[0].data, true;
    }
    return false;
  }
  initCascade() {
  ...
  touch(t) {
    ...
    switch (t.type) {
      case "touchstart":
      case "mousedown":
        s.style.transition = "none 0s ease-out", this.startY = Math.floor(t instanceof TouchEvent ? t.touches[0].clientY : t.clientY), this.preMoveY = this.startY, t.type === "mousedown" && (this.enableClickStatus = true);
        break;
      case "touchmove":
      case "mousemove":
      ...
      case "mouseup":
        ...
        t.type === "mouseup" && (this.enableClickStatus = false), this.isCascade && this.checkRange(n, this.getIndexArr());
        break;
    }
  }
};
...
//# sourceMappingURL=mobile-select.js.map
